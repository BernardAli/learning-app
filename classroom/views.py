...
from .forms import NewCourseFrom
...
def new_course(request):
    ...
    if request.method == "POST":
        form = NewCourseFrom(request.POST, request.FILES)
        if form.is_valid():
...
def edit_course(request, course_id):
    ...
    if user != course.user:
        ...
        if request.method == "POST":
            form = NewCourseFrom(request.POST, request.FILES, instance=course)
            if form.is_valid():
            ...
            form = NewCourseFrom(instance=course)

    context = {
        'form': form,
        'course': course,
    }
    render(request, 'classroom/editcourse.html', context)
